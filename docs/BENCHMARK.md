...
### 2.9 双机召回质量测试(2026-09-01)

设计:9 个用例,每个事实带唯一标记(判分客观化,grader 检查标记是否出现在
回答中;干扰项用例同时检查错误标记不得出现;否定控制检查不得编造)。
两台 Hermes 共享同一实例/同一隔离四元组:A 机写入 → 索引等待 → 两机分别提问。
脚本:`tests/recall_quality_test.py`(write/ask 两阶段,REMOTE_SSH 驱动远端)。
...
*本机 C5 三次运行中幻觉一次(不稳定性)。

**过程中发现并修复的问题:**

| # | 问题 | 根因 | 处置 |
|---|---|---|---|
| 13 | 两机 agent.log 均显示 `registered (0 tools)` — 搜索工具从未注册 | Hermes 在 `initialize()` **之前**调 `get_tool_schemas()` 注册工具,彼时 client 未构建返回空 | ✅ 修复:`get_tool_schemas()` 惰性构建 client(凭据在 env 即乐观注册);L1 门控改为"探测前乐观、探测后按结果" |
| 14 | L0 检索**新近度淹没**:写入轮/答案轮被后续对话挤出 top-N | 免费版检索相关性弱(分数扁平 0.024~0.033,近乎按时间排序),且提问轮与查询字面重叠高、排名天然高于答案轮 | ✅ 缓解:conv 窗口 8→30、**assistant 消息优先注入**、注入行数封顶 10;prefetch 事实命中率实测 1/5 → 5/7 |

**残留发现(平台/模型侧,插件无法根治):**

1. **否定控制在远端稳定幻觉**:三次运行均编造车牌"京A·73V88"——其中
   73 来自 L3 共享画像(幸运数字),即幻觉内容会混合真实记忆碎片;
   本机同样提问则正确拒绝。同一模型不同会话行为不稳定,建议生产场景
   在 system prompt 明确"记忆中无此信息时回答不知道"
2. L0 检索相关性弱是免费版平台特性:随对话量增长,旧事实持续被新轮次
   淹没(实测写入轮跌至第 20/28 位),长期可靠召回应依赖 L1(云端抽取)
   + L3(手动/人工维护)而非 L0
3. C6 类"时效更新"用例依赖 L0 检索到最新轮次,受 #14 同样影响;L1 抽取
   完成后此场景可靠性会提升(待观察)
